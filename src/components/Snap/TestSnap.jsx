...
const TestSnap = () => {
  ...
  const getPinZoneIndex = (x, y) => {
    if (x > 210.5 && x < 258.5 && y > -7 && y < 41) return 0;
    if (x > 210.5 && x < 258.5 && y > 191 && y < 239) return 1;
    if (x > 306.5 && x < 354.5 && y > 7 && y < 55) return 2;
    if (x > 306.5 && x < 354.5 && y > 81 && y < 129) return 3;
    return null;
  };
  ...
  function deleteLine(x, y) {
    const updatedLines = lines.filter(obj => {
      return !((obj.x1 === x && obj.y1 === y) || (obj.x2 === x && obj.y2 === y));
    });
  ...
  const checkLines = () => {
    ...
    setConnectedZones((prevConnectedZones) => {
      ...
      const uniqueConnectedZones = newConnectedZones.filter((obj, index, self) => {
  ...
  const handlePointMousedown = (event, index) => {
    ...
    if (pointIsFree(x, y)) {
      if (x < 258) {   //pointIsFree, xCentru < 258(centru)
      ...
      if (currentLine1) {
        ...
        s.mouseup(function (evUp) {
          ...
          if (
            ...
            deleteLine(Number(currentLine1.node.attributes[1].value), Number(currentLine1.node.attributes[3].value));
            currentLine1.remove();
          }
      ...
      if (currentLine2) {
        ...
        s.mouseup(function (evUp) {
          ...
          if (
            ...
            deleteLine(Number(currentLine2.node.attributes[0].value), Number(currentLine2.node.attributes[2].value));
            currentLine2.remove();
          }
          checkLines();
          s.unmousemove();
  ...
  return (
    <div className="content-snap">
          ...
          <div data-slate-node="element">
            <div data-slate-node="element">
              <div
                ...
                style={{ display: "block", height: "144px", width: "252px" }}
                className="slate-image-wrapper"
              >
                <div>
                  <img
                    style={{ display: "block" }}
                    src="https://cdn.uchi.ru/uchiru-five-eleven/dded327a-cdaa-4a50-beb7-e284540c3c75"
                    alt=""
                    width="252"
                    height="144"
                  />
              ...
              >
                <div data-slate-node="element">
                  <span data-slate-node="text">
                    <span data-slate-leaf="true">
                      <span data-slate-string="true">Капля висит на листе</span>
                    </span>
                  </span>
                </div>
              </div>
            ...
            <div data-slate-node="element">
              ...
              >
                <div>
                  <img
                    style={{ display: "block" }}
                    src="https://cdn.uchi.ru/uchiru-five-eleven/2ac1e652-8e37-4343-924e-921224a58b82"
                    alt=""
                    width="252"
                    height="144"
                  />
              ...
              >
                <div data-slate-node="element">
                  <span data-slate-node="text">
                    <span data-slate-leaf="true">
                      <span data-slate-string="true">Капля падает</span>
                    </span>
                  </span>
                </div>
              </div>
      ...
      <svg ref={svgboxRef} height="100%" width="100%" className="main-svg">
        ...
        <g ref={gRef}></g>
        <Pinzone
          transformMatrix="matrix(1,0,0,1,210.5,-7)"
          onPointMousedown={(event) => handlePointMousedown(event, 0)}
          index={0}
        />
        <Pinzone
          transformMatrix="matrix(1,0,0,1,210.5,191)"
          onPointMousedown={(event) => handlePointMousedown(event, 1)}
          index={1}
        />
        <Pinzone
          transformMatrix="matrix(1,0,0,1,306.5,7)"
          onPointMousedown={(event) => handlePointMousedown(event, 2)}
          index={2}
        />
        <Pinzone
          transformMatrix="matrix(1,0,0,1,306.5,81)"
