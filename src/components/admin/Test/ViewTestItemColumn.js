...
function ViewTestItemColumn() {
  ...
  const columns_header = ["ID", "Order",      "Task", "Type", "Title","Topic",   "Edit",     "Status"];
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-test-item-column/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
