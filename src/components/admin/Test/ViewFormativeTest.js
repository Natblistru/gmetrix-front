...
function ViewFormativeTest() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/view-formative-test', { params });
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-formative-test/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
