...
function AddFormativeTestItem() {
  ...
  const handleCheckboxChange = (index, isChecked) => {
    setAdditionalData((prevAdditionalData) => {
      ...
      updatedAdditionalData[index] = {
        ...
        formative_test_id: excelData[index]?.formative_test_id || '',
        formative_test_title: excelData[index]?.formative_test_title || '',        
      ...
      if (selectedData.length > 0) {
        ...
        selectedData.forEach((selectedItem) => {
          ...
          if (foundTeacherTopic) {
            ...
            foundFirmativeTest = formativeTestList.find((item) => item.title.trim() == selectedItem.formative_test_title.trim() && item.teacher_topic_id == selectedItem.teacher_topic_id);
          ...
          if (foundFirmativeTest) {
            selectedItem.formative_test_id = foundFirmativeTest.id;
          }
          else {
            notFoundFormativeTest.push(selectedItem.formative_test_title);
          }
        });

        if(notFoundTeacherTopics.length === 0 && notFoundTestItem.length === 0 && notFoundFormativeTest.length === 0)  {
          ...
          const formDataArray = selectedData.map(selectedItem => {
            ...
            formData.append('formative_test_id', selectedItem.formative_test_id);
            formData.append('status', 0); 
            return formData;
          });
          // console.log(formDataArray)
          // Trimitem fiecare set de date către server utilizând axios.all
          axios.all(formDataArray.map(formData => axios.post('http://localhost:8000/api/store-formative-test-item', formData)))
  ...
  const submitTeacherTopic = (e) => {
    ...
    axios.post(`http://localhost:8000/api/store-formative-test-item`, formData).then(res => {
...
export default AddFormativeTestItem;
