...
function EditFormativeTest(props) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/api/edit-formative-test/${formativeTest_id}`).then(res=>{
      if(res.data.status === 200){
  ...
  const submitTeacherTopic = (e) => {
    ...
    const formativeTest_id = props.match.params.id;
    axios.post(`http://localhost:8000/api/update-formative-test/${formativeTest_id}`, formData).then(res => {
      if(res.data.status === 200)
      {
        Swal.fire({
          title: "Succes",
...
export default EditFormativeTest;
