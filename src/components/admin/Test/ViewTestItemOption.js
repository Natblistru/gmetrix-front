...
function ViewTestItemOption() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('/api/view-test-item-option', { params });
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-test-item-option/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
