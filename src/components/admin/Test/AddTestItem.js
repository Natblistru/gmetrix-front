...
function AddTestItem() {
    ...
    if (excelData && excelData.length > 0) {
      ...
      if (selectedData.length > 0) {
        ...
        if(notFoundTeacherTopics.length === 0 && notFoundTestComplexity.length === 0)  {
          ...
          axios.all(formDataArray.map(formData => axios.post('http://localhost:8000/api/store-test-item', formData)))
  ...
  const submitTeacherTopic = (e) => {
    ...
    axios.post(`http://localhost:8000/api/store-test-item`, formData).then(res => {
  ...
  return (
    <div className="container-fluid px4">
      <h2 className="m-3">Add Test Item
