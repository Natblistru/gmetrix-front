...
function AddTestItemColumn() {
    ...
    if (excelData && excelData.length > 0) {
      ...
      if (selectedData.length > 0) {
        ...
        if(notFoundTeacherTopics.length === 0 && notFoundTestItem.length === 0)  {
          ...
          axios.all(formDataArray.map(formData => axios.post('/api/store-test-item-column', formData)))
  ...
  const submitTeacherTopic = (e) => {
    ...
    axios.post(`/api/store-test-item-column`, formData).then(res => {
  ...
  return (
    <div className="container-fluid px4">
      <h2 className="m-3">Add Test Item Column
...
export default AddTestItemColumn;
