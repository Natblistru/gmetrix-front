...
function ViewTestItem() {
  ...
  const columns_header = ["ID", "Task", "Type", "Complexity",     "Topic",   "Edit",     "Status"];
  const columns =        ['id', 'task', 'type', 'name',       'topic_name', 'editLink', 'status'];
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/view-test-item', { params });
          if (response.data.status === 200) {
            // console.log(response.data)
            setTeacherTopicList(response.data.testItem)
