...
function AddSubtopicImage() {
  ...
  const handleCheckboxChange = (index, isChecked) => {
    setAdditionalData((prevAdditionalData) => {
      ...
      updatedAdditionalData[index] = {
        ...
        subtopic_name: excelData[index]?.subtopic_name || '',
        subtopic_id: excelData[index]?.subtopic_id || '',
    ...
    if (excelData && excelData.length > 0) {
      ...
      if (selectedData.length > 0) {
        ...
        selectedData.forEach((selectedItem) => {
          ...
          let foundSubopic = null;
          if (foundTeacherTopic) {
            selectedItem.teacher_topic_id = foundTeacherTopic.id;
            foundSubopic = subtopicList.find((subtopic) => subtopic.name.trim() == selectedItem.subtopic_name.trim() && subtopic.teacher_topic_id == selectedItem.teacher_topic_id);
          }
          else {
            notFoundTeacherTopics.push(selectedItem.teacher_topic_name);
          }
          if (foundSubopic) {
            selectedItem.subtopic_id = foundSubopic.id;
          }
          else {
            notFoundSubtopics.push(selectedItem.subtopic_name);
          }

        });

        if(notFoundTeacherTopics.length === 0 && notFoundSubtopics.length === 0)  {

          const formDataArray = selectedData.map(selectedItem => {
            const formData = new FormData();
            formData.append('path', selectedItem.image_path);
            formData.append('subtopic_id', selectedItem.subtopic_id );
          ...
          if(notFoundTeacherTopics.length > 0 ){
            Swal.fire({
              title: "Unfound teacher topic name:",
          ...
          if(notFoundSubtopics.length > 0 ){
            Swal.fire({
              ...
              text: Object.values(notFoundSubtopics).flat().join(' '),
              icon: "error"
            });
          }
        }
  ...
  const submitTeacherTopic = (e) => {
    ...
    formData.append('path',subtopicInput.image_path );
...
export default AddSubtopicImage;
