...
function ViewSubtopicImage() {
  ...
  const columns_header = ["ID", "Path", "Subtopic", "Topic",  "Edit",     "Status"];
  const columns =        ['id', 'path', 'name', 'topic_name', 'editLink', 'status'];
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('/api/view-subtopic-image', { params });
          if (response.data.status === 200) {
            // console.log(response.data)
            setTeacherTopicList(response.data.subtopicImage)
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-subtopic-image/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
    'path': (item) => (item.path && (
      item.path.startsWith('uploads/imageSubtopic/') ? (
        <img src={`/${item.path}`} width='200px' alt={item.path || ''} />
        ) : (
          <span>{item.path || ''}</span>
        )
    )),
