...
function ViewFlipCard() {
  ...
  const columns_header = ["ID", "Task", "Topic",      "Edit",     "Status"];
  const columns =        ['id', 'task',  'topic_name','editLink', 'status'];
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/view-flip-card', { params });
          if (response.data.status === 200) {
            // console.log(response.data)
            setTeacherTopicList(response.data.flipCard)
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-flip-card/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
