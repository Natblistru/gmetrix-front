...
function ViewTeacherTopic() {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/view-teacherTopics', { params });
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-teacher-topic/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
  ...
  return (
    <div className="containerBts px-4">
      ...
      <div className="rowBts mx-4 mt-2">
        ...
        <Link to="/admin/add-teacher-topic" className="btnBts btn-primary text-white float-end">Add Teacher Topic</Link>
        </div>
      </div>
        {blockFilterVisible && 
          <div className="rowBts mx-4 mt-2">
