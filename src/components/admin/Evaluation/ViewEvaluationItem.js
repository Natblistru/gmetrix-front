...
function ViewEvaluationItem() {
  ...
  const columns_header = ["ID", "Task", "Image",      "Editable Image",      "Evaluation Subject", "Order",      "Theme", "Edit",     "Status"];
  const columns =        ['id', 'task', 'image_path', 'editable_image_path', 'title',             'order_number', 'theme_name', 'editLink', 'status'];
  const mapReactColumnToDBColumn = (reactColumnName) => {
    const columnMap = {
      'ID': 'id',
      'Task': 'task',
      'Image': 'image_path',
      'Editable Image': 'editable_image_path',
  ...
  const commonColumns = {
    ...
    'editLink': (item) => (
      <Link to={`/admin/edit-evaluation-item/${item.id}`} className="btn btn-success btn-small">
        Edit
      </Link>
    ),
  ...
  return (
    <div className="containerBts px-4">
      ...
      <div className="rowBts mx-4 mt-2">
        ...
        <Link to="/admin/add-evaluation-item" className="btnBts btn-primary text-white float-end">Add Evaluation Item</Link>
        </div>
      </div>
        {blockFilterVisible && 
          <div className="rowBts mx-4 mt-2">
