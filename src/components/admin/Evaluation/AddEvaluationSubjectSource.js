...
function AddEvaluationSubjectSource() {
    ...
    if (excelData && excelData.length > 0) {
      ...
      if (selectedData.length > 0) {
        ...
        if(notFoundEvaluationSubject.length === 0 && notFoundEvaluationSource.length === 0) {
          ...
          axios.all(formDataArray.map(formData => axios.post('/api/store-evaluation-subject-source', formData)))
  ...
  const submitEvaluationSource = (e) => {
    ...
    axios.post(`/api/store-evaluation-subject-source`, formData).then(res => {
