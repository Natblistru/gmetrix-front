...
function ViewEvaluationAnswerOption() {
  ...
  const columns_header = ["ID", "Task",       "Label", "Points",      "Evaluation Item","Evaluation Subject","Theme",  "Edit",     "Status"];
  const columns =        ['id', 'task','label', 'points','item_task','title','theme_name', 'editLink', 'status'];
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-evaluation-answer-option/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
  ...
  return (
    <div className="containerBts px-4">
      ...
      <div className="card-title m-3">
        <h2>Evaluation Answer Option List</h2>
      </div>
      <div className="rowBts mx-4 mt-2">
        ...
        <Link to="/admin/add-evaluation-answer-option" className="btnBts btn-primary text-white float-end">Add Evaluation Answer Option</Link>
        </div>
      </div>
        {blockFilterVisible && 
          <>
