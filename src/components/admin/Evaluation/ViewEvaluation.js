...
function ViewEvaluation() {
  ...
  const columns_header = ["ID", "Year", "Disciplina", "Type", "Edit",     "Status"];
  const columns =        ['id', 'year', 'name', 'type', 'editLink', 'status'];
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-evaluation/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
  ...
  return (
    <div className="containerBts px-4">
      ...
      <div className="rowBts mx-4 mt-2">
        ...
        <Link to="/admin/add-evaluation" className="btnBts btn-primary text-white float-end">Add Evaluation</Link>
        </div>
      </div>
        {blockFilterVisible && 
          <div className="rowBts mx-4 mt-2">
