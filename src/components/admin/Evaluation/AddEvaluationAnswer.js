...
function AddEvaluationAnswer() {
  ...
  const handleCheckboxChange = (index, isChecked) => {
    setAdditionalData((prevAdditionalData) => {
      ...
      updatedAdditionalData[index] = {
        ...
        max_points: excelData[index]?.max_points || '',
      ...
      if (selectedData.length > 0) {
        ...
        if(notFoundEvaluationSubject.length === 0 && notFoundEvaluationItem.length === 0) {
          ...
          const formDataArray = selectedData.map(selectedItem => {
            ...
            formData.append('max_points', selectedItem.max_points );
  ...
  return (
    <div className="container-fluid px4">
      <h2 className="m-3">Add Evaluation Answer
...
export default AddEvaluationAnswer;
