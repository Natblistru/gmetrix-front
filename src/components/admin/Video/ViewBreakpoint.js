...
function ViewBreakpoint() {
  ...
  const columns_header = ["ID", "Title", "Time", "Video", "Edit",    "Status"];
  const columns =        ['id', 'name', 'time', 'title', 'editLink', 'status'];
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8000/api/view-breakpoints', { params });
          if (response.data.status === 200) {
            // console.log(response.data)
            setTeacherTopicList(response.data.breakpoints);
  ...
  const commonColumns = {
    'editLink': (item) => (
      <Link to={`/admin/edit-breakpoint/${item.id}`} className="btnBts btn-success btn-small">
        Edit
      </Link>
    ),
  ...
  return (
    <div className="containerBts px-4">
      ...
      <div className="rowBts mx-4 mt-2">
        ...
        <Link to="/admin/add-breakpoint" className="btnBts btn-primary text-white float-end">Add Breakpoint</Link>
        </div>
      </div>
        {blockFilterVisible && 
          <div className="rowBts mx-4 mt-2">
